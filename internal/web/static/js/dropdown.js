...
//
// Upstream heroui-go'dan farkı: dropdownOpen sonunda bir viewport-clamp
// eklendi (aşağıda işaretli blok) — orijinali popover'ı her zaman trigger'ın
// sol kenarına hizalar, bu da dar (mobil) ekranlarda sağa yaslı bir
// trigger'da (ör. bir kartın kebab menüsü) popover'ı viewport'un dışına
// taşırıyordu.
...
function dropdownOpen(root) {
  ...
  // Viewport clamp (bkz. üstteki not): popover'ı ölçüp sayfanın sağından
  // taşıyorsa sola kaydır.
  var overflowRight = popover.getBoundingClientRect().right - document.documentElement.clientWidth;
  if (overflowRight > 0) {
    popover.style.left = parseFloat(popover.style.left) - overflowRight - 8 + "px";
  }
